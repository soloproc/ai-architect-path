...
- **需要安装依赖吗**：不需要。M9 纯标准库，Python ≥ 3.11 直接跑。
- **`Ran 0 tests`**：在 m9 根目录跑 `python -m unittest discover -s tests -v`。
- **`reports/` 里的 json 是什么**：demo 的运行产物（基线/候选/Gate 报告），
  删掉后重跑 `python demo.py` 会重新生成且内容逐字节一致（确定性保证）。
- **Gate 决策不是 BLOCK**：说明改动过 `evals/sut.py` 里 v1.1 的行为脚本——
  恢复原样，BLOCK 是本演示的教学结论（质量提升不能交易安全底线）。
