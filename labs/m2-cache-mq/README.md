...
- **`ModuleNotFoundError: No module named 'fakeredis'`**：没装依赖。
  仓库根 `source .venv/bin/activate && pip install -r m2-cache-mq/requirements.txt`。
- **读接口报 `no such table: orders`**：只 seed 了主库、没复制从库——
  读走从库，先跑 `python scripts/replicate.py`（这正是"写后读/读写分离"的第一课）。
- **`/cache/stats` 命中率不动**：先对同一订单连查两次再看 stats；
  每次重启服务命中率清零（fakeredis 是内存实现）。
- **`Address already in use`**：换端口 `uvicorn app.main:app --port 8001`。
- **想重置环境**：删掉 `retailhub_master.db` / `retailhub_slave.db`，
  重跑 seed + replicate。
