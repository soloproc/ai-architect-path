...
- **`Ran 0 tests`**：没在 m4 根目录跑。`cd m4-traffic-observability` 后再
  `python -m unittest discover -s tests -v`。
- **demo.py 的统计数字和 README 不完全一样**：故障窗口基于真实时钟，
  不同机器 timing 略有差异，属正常；看"三类防护都有计数"这个结构即可。
- **`python3: command not found`**：macOS 先 `xcode-select --install` 或 `brew install python3`。
- **outbox 演示想重跑**：删掉 `src/outbox_demo.db` 再跑 `python src/outbox.py`。
