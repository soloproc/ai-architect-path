...
- 订单时间均匀分布在 2025-06-01 ~ 2025-06-30，状态约 1/10 为 refunded，
  用来支撑"GMV 不计退款"口径决策（见 m1-monolith/docs/adr/0001）。
"""
import random
from datetime import datetime, timedelta

from .db import get_master, init_db
...
def seed() -> None:
    ...
    if not table_empty("orders"):
        print("已有数据，跳过 seed（如需重建请先删除 retailhub_master.db）")
        return
    ...
    conn = get_master()  # 种子数据写主库，从库由 scripts/replicate.py 复制
    try:
