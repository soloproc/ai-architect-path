# 卷04-1：服务间通信与流量治理——从"能跑"到"扛得住"（上）

> 导语：卷03 结束时，RetailHub 已经有一份数据架构设计文档，并且用防超卖实验亲手验证了"一致性是要花代价的"。但系统本质上还是卷02 拆出来的几个服务各自为战：服务之间靠写死的 IP 互调、下游一挂上游跟着雪崩。本页是卷04 的上篇，先把两个最工程化的问题解决掉：服务怎么找到彼此（RPC vs REST、服务发现、负载均衡），以及流量怎么被驯服（超时/重试/熔断/限流/降级——每一种都防一种确定的故障）。本页知识主线来自周志明《凤凰架构》的"分布式治理"视角。

> **📖 本卷怎么读（卷04 是全路径难度峰值 8/10，已拆成三页）**
> - **卷04-1（本页）· 通信与流量治理**：工程味最浓、最易上手——每种治理机制都对应一种确定的故障，配两个交互 Demo，先在这里建立"故障 → 机制"的映射直觉。
> - **卷04-2 · 分布式事务与共识**：理论峰值所在——2PC/TCC/Saga/本地消息表 + Raft 专题。不要求一遍吃透，Raft 标注"可二刷"；第一遍的重点是记住四种事务方案的取舍表和"多数派相交"一句话。
> - **卷04-3 · 可观测性、容器化与平台工程**：视野最宽——从可观测三支柱、K8s 最小集一路站到平台工程（PI 架构）与金山 KAE 案例，是通往卷06 与阶段九的世界观。
> - **建议顺序与心态**：按 04-1 → 04-2 → 04-3 顺读；在 04-2 卡住不要停超过一个晚上，先带着问题进 04-3，后面的实战会帮你回头理解。本卷实战被拆成三部分跟随各页（本页第 1-2 步、04-2 第 3 步、04-3 第 4-5 步），合起来才是完整的"RetailHub 演进第 4 步"。
...
## 8. 本卷实战（上）：给订单服务装上"安全气囊"

### 8.1 背景

卷03 末态：RetailHub 有网关 + 订单服务 + 库存服务（REST 互调）、PostgreSQL 主从、Redis 缓存、MQ 异步报表，完成了数据架构文档与防超卖实验。卷04 的实战按主题拆到三个子页，本页完成前两步：给跨服务调用装上超时与熔断、加上租户级限流——先让订单服务在下游故障与流量尖峰面前不被拖死。

### 8.2 任务清单（本页两步，验收通过再进下一步）
...
➡️ 下一页：[卷04-2：分布式事务与共识——2PC、TCC、Saga、本地消息表与 Raft](卷04-2-分布式事务与共识.md)
