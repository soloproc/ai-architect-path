# 篇05-1：Agent Harness 与 Runtime（上）——混合架构、ReAct 循环与 Run 执行模型

> 导语：在篇04 的 LLM Runtime 解决了"模型调用"的工程化（重试、超时、限流、成本、观测）之后，本页回答 DataAgent 的下一个生死问题：**模型什么时候能自主、自主到什么程度、什么时候必须停**。这是 Agent Harness（智能体挽具）的职责——包裹在模型外面的那层控制结构：计划、验证、停止条件、预算、人工关卡。很多团队的 Agent 项目死在同一个地方：Demo 阶段一个 `while` 循环加几行 `messages.append()` 跑得很欢，一进生产就被无限循环、状态丢失、重复扣款、审批缺失打死。本页（篇05 上半篇）讲三件事：混合架构选型（先定义"什么算完成"的 Task Contract 与控制权矩阵）、从 while 循环到 Autonomous Harness（ReAct 规划循环、Plan / Replan / Verifier、五类停止条件、Analysis State）、统一执行模型（Run / Step / Attempt / Event / Checkpoint 与 Run 状态机）[^geektime]。Durable Workflow 收口、分布式 Runtime 与故障工程，在下一页（篇05-2）继续。
...
➡️ 下一页：篇05-2 · Agent Harness 与 Runtime（下）——Durable Workflow、分布式 Runtime 与故障工程
