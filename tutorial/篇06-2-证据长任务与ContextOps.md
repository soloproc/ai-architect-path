# 篇06-2：Agent 上下文治理（下）——证据体系、长任务与 ContextOps

## 导语

上半篇解决了"装配什么、怎么装"，下半篇回答三个更硬的问题：信息从哪来、长跑怎么不丢、出了错怎么交代。本子页先划清 RAG 与上下文工程的边界——检索只是装配管线的候选生产者之一，并用假设/证据/结论三分法构建可追溯的证据链（第 4 节）；再治理数小时长任务的上下文——History/State/Checkpoint/Summary 分治、五种压缩手段的决策树与五条压缩保留不变量、漂移检测（第 5 节）；最后落地 Context Security 与 ContextOps：信任三圈防注入、跨租户泄漏面、Decision Trace、六类故障归因与 Replay 回归（第 6 节）。篇末实战把整条装配管线落成代码，用埋雷候选集验证六阶段行为 [^geektime]。
...
➡️ 下一页：篇07-1 · 工具与技能治理（上）——能力形态、Tool Contract 与受控执行
